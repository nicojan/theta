...
## Story button row: pinned left on purpose (2026-09-24, 447)

Nico saw the row sometimes on the left edge instead of the right, and sometimes missing on the first story after launch. A post-layout geometry log (`StoryOverlay: geometry …`) showed every build landing at `btnX=8` in a 440-wide cell — LTR, identity transform, in a window, and no Auto Layout conflict — even though the stack was constrained trailing. So the left placement came from something outside the constraints (most likely Instagram's own layout setting the frame), and the stack flipped right whenever the constraints won. RTL and a narrow cell were both ruled out by that capture. Nico prefers the left, so the stack is now pinned `leftAnchor + 8`; both mechanisms agree and it stayed left on every story in `logs/theta-20260924-195608.log`. The geometry log stays in and now reports `geometry RIGHT` as the anomaly. The missing-on-first-story symptom did not reproduce in two cold launches (the overlay built on the first story both times); if it returns, the `StoryOverlay: no viewModel` / `no owner` / `buttons=0` lines will say which link broke. Also closed: the eye long-press fires and presents its menu (`StorySeen: long-press fired, presenting menu`).

## Home-feed reels dead (open 2026-09-06; narrowed 2026-09-14; still present on 447, measured 2026-09-15; level-2 control starved too, 2026-09-24)

> **2026-09-24 — the level-2 control is starved too, so Theta's feature hooks are not the cause.** `logs/theta-20260924-150301.log`, cut at the control2b cold launch (15:09:14, `[Theta] CONTROL BUILD level 2: no feature hooks installed (TestFlight nag shim only)`): 96 running samples, 70 starved (73%), two fully-starved renderers. One created 10 s after launch runs **204 s** at rate 1.00 with zero frames; another runs 168 s. The 447 feature build's longest was 144 s. Same symptom, no feature hooks installed, so it lies upstream of Theta's hooking. **What this does not clear:** the control still has the dylib injected, the sideload keychain/container shims, the TestFlight-nag shim and a re-signed bundle, and a clean stock control is impossible (see below). The honest verdict is "not Theta's feature code". If the reel bug is still worth chasing, the remaining suspect is the re-signed sideload environment itself, and the only way to test that is a build that injects nothing at all but can still log in, which is not currently possible.
>
> Two things changed in the control to get this run. Plain control2 hit Instagram's `TestFlightUpdateNudgeViewController` ("It's time to update Instagram Beta") and could not reach the feed, because level 2 returned before registering `THRegisterHideTestFlightNagHooks()`. Level 2 now keeps that one shim (`artifacts/Instagram_447_control2b.ipa`). And every in-place install failed with `MIInstallerErrorDomain 35`, `Info.plist missing at …/PlugIns/InstagramNotificationExtension.appex` — a `./sideload` bug, not a damaged device install; see "App extensions are stripped at install".
...
Two `AVSampleBufferVideoRenderer`s were created at 13:04:13.36 with host layers, CAContextIDs and control timebase **rate 1.00** — set up and nominally playing — then received **zero samples** for the next 4.5 minutes and were never torn down. Starved renderers, not broken ones: a poster frame with nothing behind it. That is what "static" looks like from the log side.

Instagram opened its last network connection at 13:04:11.66 and cancelled it at 13:05:16.72, then **made no further connection at all**, including across three reel taps at 13:08:32.95, 13:08:34.31 and 13:08:35.70. Each tap was delivered to `<IGWindow: 0x125c18000>` and produced `send gesture actions` ~360 ms later, followed by no work of any kind — no view controller transition, no media load, not one preference read. From ~13:06 the process emits only a perfectly periodic 30-second timer heartbeat.

### Ruled out by evidence — do not re-test

- **Load Banner**, and the off-main-thread UIKit bug behind it — turned off, reels still dead. Real bug, fixed separately (64aa757), unrelated.
- **All seven feed-filter toggles** (Hide Suggested Reels/Posts, People You May Know, Threads Carousel, Home Stories, Disable Ads, Mute Entire Home Feed) — turned off together, no change.
- **Hide Explore Grid** — was already off throughout.
- **ffmpeg / libavutil symbol collision** — ffmpeg is never loaded in a normal session; zero `libav*` lines in 200 MB.
- **The process-wide `strlen` rebind** in `THSideloadFishhook.m` — `safe_strlen_impl` is semantically correct (null→0, else delegates to the real one). A performance cost across every library in the process, not a correctness one.
- **Hook binding** — one miss only, `Class is nil for _didPressFollowButton`. Nothing feed- or video-related fails to install.
...
Six hypotheses were falsified here by toggling settings, all of them generated by reading code. The project's own lesson applies and was ignored for too long: these get cracked by capturing a tap, not by reading code. Reach for instrumentation earlier — and check that the instrumentation is actually visible in a capture before trusting its silence (see Reading device logs).
...
`fprintf(stderr, …)` reaches nothing. On a sideloaded app stderr is not wired to the syslog relay, so anything written that way is invisible in a capture — it is not merely unredacted, it is absent. Three diagnostics sat behind it until 2026-09-06, including **the hook-install-miss report**, which is the first thing worth reading when a feature silently does nothing. All four sites now use `os_log` with `%{public}s` (2474ad5). The removed comment claimed "fprintf bypasses os_log privacy redaction" — true, and useless, because it bypasses the log itself.

How to tell the two failure modes apart: a redacted `os_log`/`NSLog` line still *appears*, carrying `<private>` where the value should be. An `fprintf` line does not appear at all. `[Theta] sideload keychain access group: …` is absent from every capture up to 2026-08-19 and present from `theta-20260906-135939.log` onward, while the `NSLog` lines around it appear in both — that contrast is the proof.
...
**Stripping must also prune `SC_Info/Manifest.plist` (fixed 2026-09-24).** The FairPlay manifest's `SinfReplicationPaths` lists a sinf inside each of the seven `PlugIns/*.appex`. On an **upgrade**, installd replicates sinfs to the paths listed in the *installed* app's manifest and aborts when the bundle there is gone: `MIInstallerErrorDomain 35`, `Info.plist missing at …/PlugIns/InstagramNotificationExtension.appex/Info.plist`, surfacing from `./sideload` as a bare "Failed to install the app". A fresh install does not check, so the first install always worked and every later one failed — which read as a damaged install and cost two uninstalls (login and all Theta settings) before the manifest was found. `strip_extensions` now drops those entries. Because the check reads the *old* app's manifest, one fresh install with a pruned manifest was needed to get clear; confirmed by an in-place `./sideload install` over it succeeding. Diagnose a future install failure with `--keep-unpacked` and `xcrun devicectl device install app` on the kept bundle: the wrapper swallows the recovery suggestion that names the path.
...
**The cache does not cover every hot key.** An unfiltered capture on 2026-09-06 logged `Date Format_SegmentIndex` read **161 times in 90 seconds**. `ENABLED()` only wraps boolean `…_Enabled` settings; segment-index settings are read straight off `NSUserDefaults` at their call sites (`DateFormat.m`, `TapControls.m`), so they never enter the cache. Open, unmeasured for heat.
