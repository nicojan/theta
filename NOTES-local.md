...
>
> Two things changed in the control to get this run. Plain control2 hit Instagram's `TestFlightUpdateNudgeViewController` ("It's time to update Instagram Beta") and could not reach the feed, because level 2 returned before registering `THRegisterHideTestFlightNagHooks()`. Level 2 now keeps that one shim (`artifacts/Instagram_447_control2b.ipa`). And the in-place install over the old build failed with `MIInstallerErrorDomain 35`, `Info.plist missing at …/PlugIns/InstagramNotificationExtension.appex`: the *installed* bundle carried a broken extension directory. Only an uninstall cleared it, which wipes login and every Theta setting.
